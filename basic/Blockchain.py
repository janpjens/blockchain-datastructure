...
def calculate_hash(index, previous_hash, timestamp, data):
    ...
    logger.debug('calculate_hash(%s, %s, %s, %s) => sha256(%s) => %s', index, previous_hash, timestamp, data, bstring,
                  sha256_hash)
    return sha256_hash
...
class Chain:
    ...
    def add_block(self, block):
        if self.valid_block_relation(self.get_latest_block(), block):
            ...
            logger.debug('Block #%s added with hash %s', block.index, block.hash)
        else:
            logger.error('Integrity failure when trying to add block with data %s', block.data)
    ...
    def valid_block_relation(self, former_block, latter_block):
        ...
        if latter_block.previous_hash != former_block.hash:
            logger.error('Relation hash check failed when checking block relation (%s is not %s)',
        ...
        for block in [former_block, latter_block]:
            ...
            if new_hash != block.hash:
                ...
                logger.debug('Offending block: [%s, %s, %s, %s]')
                return False
    ...
    def valid_block_chain(self, from_index=0):
        ...
        for latter_block in self.chain[from_index:]:
            ...
            logger.debug('Checking block relation between #%s and #%s', former_block.index, latter_block.index)
...
if __name__ == '__main__':
    ...
    logger.info('Blockchain initiated with genesis block')
    logger.debug('Initiated blockchain, genesis = [%s, %s, %s, %s] [%s]', genesis_block.index, genesis_block.previous_hash,
    ...
    for i in range(1, 10):
        ...
        # logger.info('Add: {prev} => {hash}'.format(prev=new_block.previous_hash, hash=new_block.hash))
        blockchain.add_block(new_block)

    logger.info('Blocks have been added, now %s blocks on blockchain', len(blockchain.chain))
    ...
    logger.info('Corrupting data for random block #%s', random_block.index)
